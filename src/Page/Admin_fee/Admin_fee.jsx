...
const Admin_fee = () => {
  ...
  const filteredFees = feeRecords.filter((fee) => {
    ...
    const matchesCourse =
      filterCourse === "All" || fee.subject === filterCourse;
  ...
  const uniqueCourses = ["All", ...new Set(feeRecords.map((f) => f.subject))];
  ...
  const totalDue = feeRecords.reduce((sum, f) => sum + (f.dueAmount || 0), 0);
