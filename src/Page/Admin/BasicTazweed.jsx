// src/Page/Admin/BasicTazweed.jsx
...
const BasicTazweed = () => {
  ...
  // ✅ Empty form template — সব field সহ
  const emptyForm = {
    studentId: "",
    name: "",
    phone: "",
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE}/basic-tazweed/all`);
  ...
  const handleAddStudent = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/basic-tazweed/add`, {
        method: "POST",
  ...
  const handleEditStudent = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/basic-tazweed/update/${editingStudent._id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteStudent = async (id, name) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/basic-tazweed/delete/${id}`, {
        method: "DELETE",
      });
      const data = await response.json();
  ...
  const handleInputChange = (e, isEdit = false) => {
    ...
    if (isEdit) {
      setEditingStudent((prev) => ({ ...prev, [name]: finalValue }));
    } else {
      setNewStudent((prev) => ({ ...prev, [name]: finalValue }));
    }
  };
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div>
            ...
            <input
              ...
              onChange={onChange}
              placeholder="TET26FB60XX"
              className="w-full px-2 py-1.5 text-xs border rounded-lg focus:ring-2 focus:ring-green-500"
            />
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-700 mb-0.5">
      ...
      <div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={onChange}
            rows="2"
            placeholder="Write any comments or notes..."
            className="w-full px-2 py-1.5 text-xs border rounded-lg focus:ring-2 focus:ring-green-500"
          />
        </div>
      </div>
    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="md:hidden bg-white border-b border-gray-200 p-3 flex justify-between items-center w-full absolute top-0 left-0 z-40">
          <h1 className="text-sm font-bold text-gray-800">Basic Tazweed</h1>
          <button
          ...
          <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-200 mb-3 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div>
              ...
              <p className="text-xs text-gray-500">
                Manage Basic Tazweed course payments & students (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white text-[10px] px-3 py-1.5 rounded-lg font-bold transition-all shadow-sm flex items-center gap-1"
              >
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-2 mb-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-7 pr-2 py-1.5 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
              />
            </div>
          </div>
          ...
          {filteredStudents.length === 0 && !loading && (
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-8 text-center mt-3">
              ...
              {students.length === 0 && (
                <button
                  onClick={() => setShowAddModal(true)}
                  className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-xs font-semibold inline-flex items-center gap-1"
                >
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white z-10">
              <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                <FaPlusCircle className="text-green-600" /> Add New Student
              </h3>
              <button
      ...
      {showEditModal && editingStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white z-10">
              <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                <FaEdit className="text-green-600" /> Edit Student
              </h3>
              <button
            ...
            <div className="p-4 border-t border-gray-200 flex gap-2 sticky bottom-0 bg-white">
              <button
                ...
                disabled={submitting}
                className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg font-semibold text-sm disabled:opacity-50"
              >
            ...
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="bg-blue-50 p-2 rounded">
                  <b>Jul-Aug:</b> ৳{selectedStudent.julyAugust}
                </div>
                <div className="bg-blue-50 p-2 rounded">
